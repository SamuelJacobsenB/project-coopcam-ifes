...
func SetupUserRoutes(rg *gin.RouterGroup, handler *user.UserHandler) {
	rg.GET("/", middlewares.RateLimiter(15), middlewares.AuthMiddlewareDriver(), handler.FindAll)
	...
	rg.POST("/", middlewares.RateLimiter(3), middlewares.AuthMiddlewareDriver(), handler.Create)
	rg.PUT("/:id/", middlewares.RateLimiter(3), middlewares.AuthMiddlewareDriver(), handler.Update)
	rg.DELETE("/:id/", middlewares.RateLimiter(3), middlewares.AuthMiddlewareDriver(), handler.Delete)
