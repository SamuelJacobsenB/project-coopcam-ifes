...
func SetupUserRoutes(rg *gin.RouterGroup, handler *user.UserHandler) {
	...
	rg.POST("/", middlewares.RateLimiter(5), middlewares.AuthMiddlewareAdmin(), handler.Create)
	rg.PUT("/:id/", middlewares.RateLimiter(10), middlewares.AuthMiddlewareAdmin(), handler.Update)
	rg.DELETE("/:id/", middlewares.RateLimiter(10), middlewares.AuthMiddlewareAdmin(), handler.Delete)
